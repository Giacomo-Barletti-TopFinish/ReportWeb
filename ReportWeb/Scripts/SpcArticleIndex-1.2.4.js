...
function getStagEvenValues()
{
    ...
    for (i = 0; i < trs.length; i++)
    {
        ...
        var tdStatoProt = $(tr).find('td')[3];
        var statoProt = $($(tdStatoProt).children()[0]).val();

        var tdCOProt = $(tr).find('td')[4];
        var coProt = $($(tdCOProt).children()[0]).val();

        var camp = $(tr).find('td')[5];
        var campChecked = $($(camp).children()[0]).prop('checked');

        var tdStatoCamp = $(tr).find('td')[6];
        var statoCamp = $($(tdStatoCamp).children()[0]).val();

        var tdCOCamp = $(tr).find('td')[7];
        var coCamp = $($(tdCOCamp).children()[0]).val();

        var prod = $(tr).find('td')[8];
        var prodChecked = $($(prod).children()[0]).prop('checked');

        var tdStatoProd = $(tr).find('td')[9];
        var statoProd = $($(tdStatoProd).children()[0]).val();

        var tdCOProd = $(tr).find('td')[10];
        var coProd = $($(tdCOProd).children()[0]).val();

        var stagEven = {
            'Stag': stag, 'Even': even, 'StagDesc': stagDesc, 'EvenDesc': evenDesc, 'Prot': protChecked,
            'Camp': campChecked, 'Prod': prodChecked, 'statoProt': statoProt, 'statoCamp': statoCamp, 'statoProd': statoProd,
            'coProt': coProt, 'coCamp': coCamp, 'coProd': coProd
        };
        data.push(stagEven);
    }
    return data;
...
function setStagEvenValues(dataString)
{
    ...
    for (i = 0; i < data.length; i++)
    {
        ...
        var statoProt = data[i].statoProt;
        var statoCamp = data[i].statoCamp;
        var statoProd = data[i].statoProd;
        var coProt = data[i].coProt;
        var coCamp = data[i].coCamp;
        var coProd = data[i].coProd;

        if (i > 0)
            fontbold = '';
        var markup = getMarkup();
        var select = '<select class="form-control input-sm articolo-modificato disabilitaVersione spcSelectEvalStati">' + markup + '</select>';

        var selectCO = "<select class='form-control input-sm articolo-modificato disabilitaVersione'><option value=''></option><option value='NC'>Non copiare</option><option value='CO'>CO</option></select>";
        ...
        tr += '<td width="14%">' + selectedStagText + '</td>';
        tr += '<td width="14%">' + selectedEventText + '</td>';
        tr += prot ? '<td width="8%" align="center"><input class="disabilitaVersione" type="checkbox" name="prot" checked onclick="paginaModificata=true;" /></td>' : '<td width="8%" align="center"><input class="disabilitaVersione" type="checkbox" name="prot" onclick="paginaModificata=true;" /></td>';
        tr += '<td width="7%" align="center">' + select + '</td>';
        tr += '<td width="8%" align="center">' + selectCO + '</td>';
        tr += camp ? '<td width="8%" align="center"><input class="disabilitaVersione" type="checkbox" name="camp" checked onclick="paginaModificata=true;"/></td>' : '<td width="8%" align="center"><input class="disabilitaVersione" type="checkbox" name="camp" onclick="paginaModificata=true;" /></td>';
        tr += '<td width="7%" align="center">' + select + '</td>';
        tr += '<td width="8%" align="center">' + selectCO + '</td>';
        tr += prod ? '<td width="8%" align="center"><input class="disabilitaVersione" type="checkbox" name="prod" checked onclick="paginaModificata=true;"/></td>' : '<td width="8%" align="center"><input class="disabilitaVersione" type="checkbox" name="prod" onclick="paginaModificata=true;" /></td>';
        tr += '<td width="7%" align="center">' + select + '</td>';
        tr += '<td width="8%" align="center">' + selectCO + '</td>';
        tr += '<td width="3%"><a class="disabilitaVersione" title="Cancella"><i class="fa fa-fw fa-remove"></i></a></td>';
        tr += '<\tr>';

        $('#validStagEvenTable').last().append(tr);

        var tr = $('#validStagEvenTable tr:last');
        var tdStatoProt = $(tr).find('td')[3];
        $($(tdStatoProt).children()[0]).val(statoProt);

        var tdStatoCamp = $(tr).find('td')[6];
        $($(tdStatoCamp).children()[0]).val(statoCamp);

        var tdStatoProd = $(tr).find('td')[9];
        $($(tdStatoProd).children()[0]).val(statoProd);

        var tdCOProt = $(tr).find('td')[4];
        $($(tdCOProt).children()[0]).val(coProt);

        var tdCOCamp = $(tr).find('td')[7];
        $($(tdCOCamp).children()[0]).val(coCamp);

        var tdCOProd = $(tr).find('td')[10];
        $($(tdCOProd).children()[0]).val(coProd);

        $('.articolo-modificato').change(function ()
        {
            paginaModificata = true;
        });
...
{
    var markup = '';
    var righe = $('#hfTabdec_StatiSchTec').val().split('!');
    for (var r = 0; r < righe.length; r++)
    {
        var elementi = righe[r].split('#');
        if (elementi.length == 2)
            markup += '<option value="' + elementi[0] + '">' + elementi[1] + '</option>';
    }
...
function AggiungiStagEvenVali()
{
    ...
    if (!ExistsStagEven(selectedStagVal, selectedEventVal))
    {
        ...
        tr += '<td width="14%">' + selectedStagText + '</td>';
        tr += '<td width="14%">' + selectedEventText + '</td>';
        tr += '<td width="8%" align="center"><input type="checkbox" name="prot" /></td>';
        tr += '<td width="7%" align="center">' + select + '</td>';
        tr += '<td width="8%" align="center">' + selectCO + '</td>';
        tr += '<td width="8%" align="center"><input type="checkbox" name="camp" /></td>';
        tr += '<td width="7%" align="center">' + select + '</td>';
        tr += '<td width="8%" align="center">' + selectCO + '</td>';
        tr += '<td width="8%" align="center"><input type="checkbox" name="prod" /></td>';
        tr += '<td width="7%" align="center">' + select + '</td>';
        tr += '<td width="8%" align="center">' + selectCO + '</td>';
        tr += '<td width="3%"><a class="disabilitaVersione" title="Cancella"><i class="fa fa-fw fa-remove"></i></a></td>';
        tr += '<\tr>';
